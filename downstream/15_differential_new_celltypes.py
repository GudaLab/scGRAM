...
# NEW multi-subtype celltypes only (single-subtype ITL34/CHO/AMY handled by
# 15_single_subtype_new.py). Clean prefixes resolve via symlink trees built by
# 15_setup_symlinks.py (ITL23/ASCT pooled across parts).
PREFIXES = [
    "ITL23", "ASCT", "L6B", "ITL4", "ITL45",
]

# 4 data sources only — uncharacterized/UNKNOWN_TFBS deliberately excluded
# (confirmed not required for differential + cross-group steps).
DATA_SOURCES = [
...
TOP_N_HEATMAP    = 25
N_WORKERS        = min(48, mp.cpu_count())   # lowered from 96 to reduce pool-hang risk

MONITOR_LOG = os.path.join(BASE_DIR, "15_new_celltypes_monitor.log")
GLOBAL_CKPT = os.path.join(BASE_DIR, "15_new_celltypes_global_checkpoint.json")
...
# ================================================================
#  ROBUST PARALLEL READ — timeout + single-thread fallback
#  Converts a hung/broken ProcessPoolExecutor (the silent-deadlock
#  failure mode) into a recoverable single-threaded pass instead of
#  blocking forever in as_completed().
# ================================================================

def parallel_read(tasks, worker_fn, prefix, ct, consume, init=None, initargs=()):
    """Run worker_fn over tasks in parallel and STREAM each result to consume()
    as it completes (results are never accumulated in a list — keeps the parent
    process small, which also keeps fork/COW cheap). A sliding window caps the
    number of in-flight results at ~2*N_WORKERS. On pool timeout/breakage it
    falls back to a single-threaded streaming pass so progress always continues."""
    n = len(tasks)
    if n == 0:
        return
    # per-result wall-clock budget for the slowest cell; generous
    step_timeout = 1800  # 30 min with no single result completing => treat as hung
    for attempt in range(2):
        try:
            kw = {}
            if init is not None:
                kw = dict(initializer=init, initargs=initargs)
            with ProcessPoolExecutor(max_workers=N_WORKERS, **kw) as pool:
                task_iter = iter(tasks)
                inflight = set()
                for _ in range(min(2 * N_WORKERS, n)):
                    inflight.add(pool.submit(worker_fn, next(task_iter)))
                done_n = 0
                while inflight:
                    done, inflight = wait(inflight, timeout=step_timeout,
                                          return_when=FIRST_COMPLETED)
                    if not done:
                        raise FuturesTimeout()
                    for fut in done:
                        consume(fut.result())   # process + discard immediately
                        done_n += 1
                        try:
                            inflight.add(pool.submit(worker_fn, next(task_iter)))
                            ...
                            pass
                    if done_n % 5000 == 0:
                        log_step(prefix, "pass", f"{ct}: {done_n}/{n} cells")
            return
        except FuturesTimeout:
            log_step(prefix, "pass-WATCHDOG",
                     f"{ct}: no result for {step_timeout/60:.0f} min "
                     f"(attempt {attempt+1}) — falling back to single-thread")
            break
        except BrokenProcessPool:
            log_step(prefix, "pass-WATCHDOG",
                     f"{ct}: BROKEN pool (attempt {attempt+1}) — retrying")
            continue
    # --- single-threaded streaming fallback (always finishes) ---
    if init is not None:
        init(*initargs)   # set module global for filtered worker in this process
    for i, t in enumerate(tasks):
        consume(worker_fn(t))
        if (i + 1) % 2000 == 0:
            log_step(prefix, "pass-FALLBACK", f"{ct}: {i+1}/{n} cells (single-thread)")
...
def run_prefix(PREFIX):
    ...
    if not step_done(OUT,"pass1"):
        ...
        # Incremental per-subtype checkpoint so a kill/restart resumes mid-pass1
        if step_done(OUT,"pass1_partial"):
            pp = load_step(OUT,"pass1_partial")
            global_counts   = defaultdict(int, pp["global_counts"])
            region_type_map = defaultdict(set, {k:set(v) for k,v in pp["region_type_map"].items()})
            cells_per_ct    = defaultdict(set, {k:set(v) for k,v in pp["cells_per_ct"].items()})
            done_cts        = set(pp["done_cts"])
            log_step(PREFIX,"pass1",f"resuming — {len(done_cts)} subtypes already read: {sorted(done_cts)}")
        else:
        ...
        for ct in all_cts:
            ...
            def _consume1(res, _ct=ct):
                cid, unified = res
                cells_per_ct[_ct].add(cid)
                for reg, tags in unified.items():
                    global_counts[reg] += 1
                    region_type_map[reg].update(tags)
            parallel_read(tasks, _process_cell, PREFIX, ct, _consume1)

            done_cts.add(ct)
            log_step(PREFIX,"pass1",f"{ct}: done — {len(cells_per_ct[ct])} cells; checkpointing")
            save_step(OUT,"pass1_partial",{
                "global_counts":dict(global_counts),
                "region_type_map":{k:list(v) for k,v in region_type_map.items()},
                "cells_per_ct":{k:list(v) for k,v in cells_per_ct.items()},
                "done_cts":sorted(done_cts),
            })
    ...
    if not step_done(OUT,"pass2"):
        ...
        # Incremental per-subtype checkpoint for pass2 as well
        if step_done(OUT,"pass2_partial"):
            pp2 = load_step(OUT,"pass2_partial")
            ct_counts = {ct:defaultdict(int, pp2["ct_counts"].get(ct,{})) for ct in celltypes}
            done_cts2 = set(pp2["done_cts"])
            log_step(PREFIX,"pass2",f"resuming — {len(done_cts2)} subtypes done")
        else:
        ...
        for ct in celltypes:
            ...
            def _consume2(res, _ct=ct):
                cid, regs = res
                cc = ct_counts[_ct]
                for reg in regs:
                    cc[reg] += 1
            parallel_read(tasks, _process_cell_filtered, PREFIX, ct, _consume2,
                          init=_init_filt_set, initargs=(filt_set,))

            done_cts2.add(ct)
            n_active = sum(1 for v in ct_counts[ct].values() if v>0)
            log_step(PREFIX,"pass2",f"{ct}: {n_active:,} active regions; checkpointing")
            save_step(OUT,"pass2_partial",{
                "ct_counts":{c:dict(d) for c,d in ct_counts.items()},
                "done_cts":sorted(done_cts2),
            })
