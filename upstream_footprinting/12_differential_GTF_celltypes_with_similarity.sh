...
#SBATCH --job-name=12_diff_GTFs_celltypes
#SBATCH --output=12_diff_GTFs_celltypes_%j.out
#SBATCH --error=12_diff_GTFs_celltypes_%j.err
...
# ================== CONFIG ==================
SUMMARY_ROOT = "/work/avinash/user/CA/CA/MSN_sorted_scBAMs_GTF_csv"
OUT_DIR = "/work/avinash/user/CA/CA/MSN_differential_GTF_celltypes"
...
def parse_sample_from_filename(base_name: str) -> str:
    ...
    Your files look like:
      MM_429_D12NAC_<barcode>_BD__merged_overlaps_GTF_summary.csv
...
def discover_files_for_celltype_dir(ct_dir: str):
    ...
    """
    patterns = [
        os.path.join(ct_dir, "*_merged_overlaps_GTF_summary.csv"),
        os.path.join(ct_dir, "*_overlaps_GTF_summary.csv"),
    ]
    files = []
    for p in patterns:
    ...
    if not files:
        patterns_old = [
            os.path.join(ct_dir, "*", "*", "summaries", "*_overlaps_GTF_summary.csv"),
            os.path.join(ct_dir, "*", "*", "summaries", "*_merged_overlaps_GTF_summary.csv"),
        ]
...
print(f"\nTotal unique GTFs across all celltypes/samples: {len(all_GTFs)}")
...
rows = []
for (chrom, start, end) in all_GTFs:
...
for N in [25, 50, 100]:
    ...
    plt.title(f"Top {topN} Differential GTFs (% Cells per Celltype)")
    plt.xlabel("Celltype")
    plt.ylabel("GTF (Chromosome:Start-End)")
    plt.tight_layout()
...
for N in [25, 50, 100]:
    if len(master_df) == 0:
        print("No GTFs; skipping volcano plots.")
        break
    topN = min(N, len(master_df))
    top = master_df.head(topN).copy()
    ...
    plt.title(f"Volcano Plot (Top {topN} Differential GTFs)")
...
# ================== 6) Exclusive GTFs per celltype (CSV + barplot) ==================
...
for ct in celltypes:
    ...
    if not sub.empty:
        excl_path = os.path.join(exclusive_dir, f"exclusive_GTFs_{ct}.csv")
        sub[cols_to_save].to_csv(excl_path, index=False)
        print(f"✅ Exclusive GTFs for {ct} saved to:\n  {excl_path}")
...
plt.ylabel("Number of Exclusive GTFs")
plt.title("Exclusive GTFs per Celltype")
plt.xticks(rotation=45, ha="right")
plt.tight_layout()

bar_excl_png = os.path.join(OUT_DIR, "exclusive_GTFs_per_celltype.png")
bar_excl_pdf = os.path.join(OUT_DIR, "exclusive_GTFs_per_celltype.pdf")
plt.savefig(bar_excl_png, dpi=300, bbox_inches="tight")
plt.savefig(bar_excl_pdf, dpi=300, bbox_inches="tight")
plt.close()
print(f"✅ Barplot of exclusive GTFs per celltype saved to:\n  {bar_excl_png}\n  {bar_excl_pdf}")
...
for ct in celltypes:
    ...
    if n_cells_ct < 2:
        print(f"ℹ️  Celltype {ct} has <2 cells with GTF data; skipping Jaccard.")
        continue
    ...
    plot_jaccard_heatmap(cell_ids_ct, mat_ct,
                         title=f"Pairwise GTF Jaccard (%) - {ct}",
                         out_prefix=out_prefix_ct)

n_global_cells = len(global_cell_enh)
print(f"\nGlobal Jaccard: {n_global_cells} cells with GTF data.")
...
if 2 <= n_global_cells <= 250:
    ...
    plot_jaccard_heatmap(cell_ids_glob, mat_glob,
                         title="Global Pairwise GTF Jaccard (%) - All Cells",
...
print("\n🎉 Step 12 complete: differential GTFs + celltype exclusives + per-subtype rankings + Jaccard similarity.")
EOF
